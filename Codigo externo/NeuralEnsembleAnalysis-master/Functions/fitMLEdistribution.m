...
for loop = 1:nfits
    switch fits(loop)
        case 1
            ...
            s = std(xdata);
            coeffs{loop} = [m, s];
            Y = (1 / (s * sqrt(2 * pi))) * exp(-((xdata - m).^2) / (2 * s^2));
            L(loop) = -sum(log(Y));
        ...
        case 2
            ...
            mu = mean(logX);
            sigma = std(logX);
            coeffs{loop} = [mu, sigma];
            Y = (1 ./ (xdata * sigma * sqrt(2 * pi))) .* exp(-((log(xdata) - mu).^2) / (2 * sigma^2));
            L(loop) = -sum(log(Y));

        case 3
            % Gamma distribution
            % Maximum Likelihood Estimation for Gamma parameters
            k = mean(xdata)^2 / var(xdata); % Shape parameter
            theta = var(xdata) / mean(xdata); % Scale parameter
            coeffs{loop} = [k, theta];
            Y = (xdata.^(k - 1) .* exp(-xdata / theta)) / (gamma(k) * theta^k);
            L(loop) = -sum(log(Y));

        case 4
            % Exponential distribution
            lambda = 1 / mean(xdata); % Rate parameter
            coeffs{loop} = lambda;
            Y = lambda * exp(-lambda * xdata);
            L(loop) = -sum(log(Y));

        case 5
            % Weibull distribution
            phat = wblfit(xdata); % Use MATLAB's internal wblfit
            coeffs{loop} = phat;
            a = phat(1); % Scale parameter
            b = phat(2); % Shape parameter
            Y = (b / a) * ((xdata / a).^(b - 1)) .* exp(-(xdata / a).^b);
            L(loop) = -sum(log(Y));

        case 6
            % Power-law distribution (requires external fit method or custom implementation)
            coeffs{loop} = pwrfit(xdata, varargin{1});
            L(loop) = pwrlike(coeffs{loop}, xdata, varargin{1});
