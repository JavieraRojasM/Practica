...
save_fname = ['DataSet_ConsensusClustering_ReduceData'];  % save-file for clustering results
...
for loop = 1:nfiles   
    ...
    %%%%%% now cluster %%%%%%%%%%%%%%%%%%%
    [Gmax,Gcon,Sxy,spkfcn] = consensus_cluster_spike_data_binless(name, thesespks,allcellIDs,T,Qt,binlessopts);
    ...
    title(sprintf('Data-set %s in increasing similarity (bottom-to-top)', name));
    
    % Ruta de la carpeta donde quieres guardar
    output_folder = "C:\Users\javie\OneDrive\Escritorio\GitHub\Practica\Codigo externo\Oficial\Results\SpikeTrainCommunitiesToolBox\Raster Plots";
    ...
    % Crear la ruta completa para el archivo .mat
    output_file = fullfile(output_folder, sprintf('%s', name));
    
    % Guardar el objeto de la figura en el archivo .mat
    savefig(output_file);    
