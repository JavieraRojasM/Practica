function [Gmax,Gcon,varargout] = consensus_cluster_spike_data_binless(name, spkdata,Didxs,T,bin,varargin)
...
for loop = 1:numel(opts.BLpars)
    ...
    %% Shows the similary matrix

    figure;
    imagesc(Sxy{1,1})
    hold on
    colormap(parula); 
    title(sprintf('Similarity matrix of %s', name));
    colorbar
    hold off
    
    % Ruta de la carpeta donde quieres guardar
    output_folder = "C:\Users\javie\OneDrive\Escritorio\GitHub\Practica\Codigo externo\Oficial\Results\SpikeTrainCommunitiesToolBox\Similarity matrix";
